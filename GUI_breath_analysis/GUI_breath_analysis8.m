function varargout = GUI_breath_analysis8(varargin)
% GUI_BREATH_ANALYSIS8 MATLAB code for GUI_breath_analysis8.fig
%      GUI_BREATH_ANALYSIS8, by itself, creates a new GUI_BREATH_ANALYSIS8
...
%      changed accordingly
%
%      H = GUI_BREATH_ANALYSIS8 returns the handle to a new GUI_BREATH_ANALYSIS8 or the handle to
%      the existing singleton*.
%
%      GUI_BREATH_ANALYSIS8('CALLBACK',hObject,eventData,handles,...) calls the local
%      function named CALLBACK in GUI_BREATH_ANALYSIS8.M with the given input arguments.
%
%      GUI_BREATH_ANALYSIS8('Property','Value',...) creates a new GUI_BREATH_ANALYSIS8 or raises the
%      existing singleton*.  Starting from the left, property value pairs are
%      applied to the GUI before GUI_breath_analysis8_OpeningFcn gets called.  An
%      unrecognized property name or invalid value makes property application
%      stop.  All inputs are passed to GUI_breath_analysis8_OpeningFcn via varargin.
%
...
% Last Modified by GUIDE v2.5 22-Jun-2015 15:23:03

% Begin initialization code - DO NOT EDIT
gui_Singleton = 1;
gui_State = struct('gui_Name',       mfilename, ...
                   'gui_Singleton',  gui_Singleton, ...
                   'gui_OpeningFcn', @GUI_breath_analysis8_OpeningFcn, ...
                   'gui_OutputFcn',  @GUI_breath_analysis8_OutputFcn, ...
                   'gui_LayoutFcn',  [] , ...
                   'gui_Callback',   []);
...
% --- Executes just before GUI_breath_analysis8 is made visible.
function GUI_breath_analysis8_OpeningFcn(hObject, eventdata, handles, varargin)
% This function has no output args, see OutputFcn.
% hObject    handle to figure
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
% varargin   command line arguments to GUI_breath_analysis8 (see VARARGIN)

% Choose default command line output for GUI_breath_analysis8
handles.output = hObject;
% Update handles structure
guidata(hObject, handles);

% UIWAIT makes GUI_breath_analysis8 wait for user response (see UIRESUME)
% uiwait(handles.figure1);


% --- Outputs from this function are returned to the command line.
function varargout = GUI_breath_analysis8_OutputFcn(hObject, eventdata, handles) 
...
[filename, pathname] = uigetfile('*.mat', 'Pick a segment');
    if isequal(filename,0) || isequal(pathname,0)
       ...
       [y_ref,y_lim] = find_pk_ref(pathname);
       handles.y_lim = y_lim;
       handles.y_ref = y_ref;
       
       %set textbox
       set(handles.textbox1, 'String', length(S.type_cell));
       set(handles.Textbox2, 'String', counter);
       set(handles.textbox4, 'String', handles.window);
       set(handles.textbox6, 'String', handles.filename);
       
       %set listbox
       if strcmp(handles.type(counter),'U')
           v = 1;
       elseif strcmp(handles.type(counter),'N')
           v = 2;
       elseif strcmp(handles.type(counter),'I')
           v = 3;
       elseif strcmp(handles.type(counter),'F')
           v = 4;
       else
           v = 5; %Low signal
       end
       ...
       %set axes1          
       axes(handles.axes1);
       if S.new_index(counter,2)-S.new_index(counter,1) > 1
           plot(S.t(S.new_index(counter,1):S.new_index(counter,2)),S.p(S.new_index(counter,1):S.new_index(counter,2)))
           ylim([0,max(S.p(S.new_index(counter,1):S.new_index(counter,2)))]);
       end
              
       %set axes 3
       axes(handles.axes3);  
       [i_s,i_e,~,c_e] = axis3_indexes_new(S.t,counter,handles.window,S.t_cell,0);%c_e is the end counter
       
%      [i_s,i_e,~,c_e] =
%      axis3_indexes(S.t,counter,handles.window,S.t_cell,0);%c_e is the end
%      counter; old command 
       ...
       ylim([-0.5*handles.y_lim,handles.y_lim]);
       yL = get(gca,'YLim');
...
if v==1
    handles.type{handles.counter} = 'U';
elseif v==2
    handles.type{handles.counter} = 'N';
elseif v==3
    handles.type{handles.counter} = 'I';    
elseif v==4 
    handles.type{handles.counter} = 'F';
else
    handles.type{handles.counter} = 'L';
end
guidata(hObject, handles);
...
%             plot(S.t_cell{ii-1},S.p_cell{ii-1})
            if S.new_index(ii-1,2)-S.new_index(ii-1,1) > 1        
                plot(S.t(S.new_index(ii-1,1):S.new_index(ii-1,2)),S.p(S.new_index(ii-1,1):S.new_index(ii-1,2)))
                ylim([0,max(S.p(S.new_index(ii-1,1):S.new_index(ii-1,2)))]);
            end
%             plot(S.t(S.new_index(ii-1,1):S.new_index(ii-1,2)),S.p(S.new_index(ii-1,1):S.new_index(ii-1,2)))
%             S.p(S.new_index(counter,1):S.new_index(counter,2))
