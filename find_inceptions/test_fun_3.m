function [new_index,type_cell] = test_fun_3(t,p,t_cell,p_cell,loop_count,r,t_c)
%new method
dp = diff(p);
...
while count <= loop_count
    ...
    for ii = 2:len_t_cell
        ...
        min_abs_slope = min(abs(dp_seg)); %find the mim absolute slope
        ind_ss = find(abs(dp_seg)==min_abs_slope); %find the loc of this point
        if length(ind_ss)>1
            ind_ss = ind_ss(end);
        end
        %assume ind_ss
