...
%Find all zero-crossings and write to CSV
% nasalPincr = nasalP(1:size(nasalP)); %vector of nasalP window
% nasalPsign =  nasalPincr > 0; %vector of 1s for nasalP > 0 and 0s for nasalP <= 0
% dif = nasalPsign(2:end)- nasalPsign(1:end-1); %find differences to identify transition from 0 to 1
% startTemp = find(dif == 1); %dif = 1 is transition from neg to pos
% endTemp = find(dif == -1); % dif = -1 is transition from pos to neg
% StartP = time(startInd) + startTemp/Fs;
% EndP = time(startInd) + endTemp/Fs;
% 

% csvwrite('startP.csv',StartP)
% csvwrite('endP.csv',EndP)

% dlmwrite('startP1.txt',StartP,'precision', 7)
% dlmwrite('endP1.txt',EndP,'precision', 7)
...
for ii = startInd:incr:lastInd;
    ...
    %de-nosing of zero-crossings
    [start_stamp, end_stamp] = event_capture(nasalPincr,i_start,i_end,Fs,min_intv,min_area);
...
%     linkaxes(ax,'x');
    ...
    hold on
    plot(t_start,nasalP(round(t_start*Fs)),'rx') %plot StartP
    plot(t_end,zeros(size(t_end)),'bo') %plot EndP
